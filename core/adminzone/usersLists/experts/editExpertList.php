...
$query = "SELECT email from t_new_experts where id = '$id';";
$email_old = mysqli_fetch_row(mysqliQuery($query))[0];
$email_old  = mb_convert_encoding($email_old,"utf-8","cp1251");

//Обновляем информацию об эксперте в БД t_new_experts
$query = "UPDATE t_new_experts set
...
//Если админ ввел пароль в input, то забирем его значение
if ($password != '') {
...
if ($checkbox != 'disabled') {
  //Проверяем не было ли пароля в базе данных (если нет, то отправляем сообщение на почту эксперту)
    ...
    if ($password64_old == null || $emailpass == null) {
        ...
        //генерируем новый пароль для пользователя если не был введен в input
        if ($password == '') {
